          ...
          if (productDetailGetOneId[0].quantity < quantity || cartId[0]?.quantity + quantity > productDetailGetOneId[0].quantity) {
            ...
            if (userStore?.current?._id) {
              await onAddCart({
                ...
                quantity: quantity,
                totalMoney: getOneProduct?.discount == 0 ? getOneProduct?.price * quantity : getOneProduct?.discount * quantity
            ...
            <form onSubmit={handleSubmit(handleFormProductDetail)}>
              <div className="px-4">
                <div className="flex items-center gap-x-[109px] py-4 mb-2">
                  ...
                  {getOneProduct?.discount === 0 ? <div className="price flex gap-x-[8px] items-baseline">
                    <span className="text-[13px] text-[#878C8F]">
                      <span className="font-bold">{getOneProduct?.price.toLocaleString("vi-VN")}đ</span>
                    </span>
                  ...
                  {getOneProduct && getOneProduct?.discount === 0 ? "" : (getOneProduct && getOneProduct?.price > getOneProduct?.discount ? <span className="width-[52px]  top-3 left-3 height-[22px] rounded-full px-3 py-[5px] text-xs font-semibold text-white bg-[#FF0000]">
            ...
            >
              {productRelated?.map((product, index) => {
                return <SwiperSlide key={index}>
                  {[...new Set(productDetailState?.filter((item) => item.product_id === product?._id).filter((pro) => pro.quantity !== 0))].length != 0 ?
